...
See step1_russell3000_filtering/config.py for details.
...
def step1_load_ticker_to_cusip(input_file: Path, handler: PostgresHandler, 
    ...
    if not cusip_col:
        print("âŒ Error: 'cusip' column not found")
    ...
    # Create table
    print(f"\nðŸ“Š Creating {table_name} table...")
    ...
    try:
        ...
        print(f"  âŒ Error inserting data: {e}")
        return {'success': False, 'error': str(e)}
...
def step2_filter_holdings_by_quarter(handler: PostgresHandler, 
    ...
    print(f"\nðŸ“… Processing {len(quarters)} quarters ({start_quarter} to {end_quarter})")
    
    # Ensure target table exists (without partitions for simplicity)
    print(f"\nðŸ“Š Preparing {target_table} table...")
    ...
    try:
        ...
        handler.connection.commit()
        cursor.close()
        print("  âœ… Table created with indexes")
    except Exception as e:
        print(f"  âŒ Error creating table: {e}")
        return {'success': False, 'error': str(e)}
    
    # Process each quarter
    print("\nðŸ” Filtering holdings per quarter...")
    print("-" * 80)
    ...
    for i, q in enumerate(quarters):
        ...
        try:
            ...
            print(f"  [{i+1}/{len(quarters)}] {quarter_str}: "
                  f"{original_count:,} â†’ {filtered_count:,} ({filter_rate:.1f}% kept) | "
                  f"Inserted: {inserted_count:,}")
            
        except Exception as e:
            print(f"  âŒ Error processing {quarter_str}: {e}")
            continue
...
def run_pipeline(input_file: Path, 
    ...
    print(f"  âœ… Connected to {handler.database}")
...
def main():
    ...
    if not input_file.exists():
        print(f"âŒ Error: File not found: {input_file}")
        sys.exit(1)
