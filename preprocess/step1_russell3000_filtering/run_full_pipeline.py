#!/usr/bin/env python
"""
Full Pipeline: Russell Index Parser â†’ Filter Holdings
...
Usage:
    python step1_russell3000_filtering/run_full_pipeline.py [options]
    
Examples:
    python step1_russell3000_filtering/run_full_pipeline.py
    python step1_russell3000_filtering/run_full_pipeline.py --skip-indices-step 6 7 8
    python step1_russell3000_filtering/run_full_pipeline.py --log-file my_log.txt
"""
...
# Import after setting up paths to ensure step1_russell3000_filtering is in sys.path
import sys
from pathlib import Path
if str(Path(__file__).parent.parent) not in sys.path:
    sys.path.insert(0, str(Path(__file__).parent.parent))

from step1_russell3000_filtering.config import (
...
def run_full_pipeline(
    ...
    """
    Run the complete pipeline: indices_parser â†’ filterholdings.
    ...
    try:
        ...
        print("=" * 80)
        print("ðŸš€ FULL PIPELINE: RUSSELL INDEX â†’ FILTERED HOLDINGS")
        print("=" * 80)
        ...
        # =====================================================================
        print("\n" + "=" * 80)
        print("ðŸ“¦ PHASE 1: INDICES PARSER PIPELINE")
        print("=" * 80)
        ...
        try:
            ...
            print(f"\nâœ… Phase 1 completed in {format_duration(time.time() - phase1_start)}")
        except Exception as e:
            print(f"\nâŒ Phase 1 failed: {e}")
            import traceback
            traceback.print_exc()
            results['indices_parser'] = {
                'success': False,
        ...
        if skip_filter:
            print("\n" + "=" * 80)
            print("â­ï¸  PHASE 2: FILTER HOLDINGS (SKIPPED)")
            print("=" * 80)
            results['filterholdings'] = {'skipped': True}
        else:
            print("\n" + "=" * 80)
            print("ðŸ“¦ PHASE 2: FILTER HOLDINGS PIPELINE")
            print("=" * 80)
            ...
            if not output_csv.exists():
                print(f"âŒ Error: Output CSV from Phase 1 not found: {output_csv}")
                ...
                try:
                    ...
                    print(f"\nâœ… Phase 2 completed in {format_duration(time.time() - phase2_start)}")
                except Exception as e:
                    print(f"\nâŒ Phase 2 failed: {e}")
                    import traceback
                    traceback.print_exc()
                    results['filterholdings'] = {
                        'success': False,
        ...
        print("\n" + "=" * 80)
        print("ðŸ“Š FULL PIPELINE SUMMARY")
        print("=" * 80)
        
        # Phase 1 status
        p1 = results['indices_parser']
        p1_status = "âœ…" if p1 and p1.get('success') else "âŒ"
        ...
        if p2 and p2.get('skipped'):
            p2_status = "â­ï¸ Skipped"
            p2_duration = ""
        else:
            p2_status = "âœ…" if p2 and p2.get('success') else "âŒ"
        ...
        print(f"\nâœ… Pipeline complete. Full log saved to: {log_file}")


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(
        description="Full Pipeline: Russell Index Parser â†’ Filter Holdings",
    ...
    if not input_dir.is_dir():
        print(f"âŒ Error: Input directory not found: {input_dir}")
        sys.exit(1)
