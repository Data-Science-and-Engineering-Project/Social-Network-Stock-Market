...
def map_cusips_to_tickers(input_file: Path, reference_file: Path, 
    ...
    print(f"  Loaded {len(ref)} CUSIPâ†’TICKER mappings from reference file")
    ...
    print(f"  â­ï¸  Already had TICKER: {already}")
    print(f"  âœ… Found {stats['reference']} NEW tickers from reference file")
    print(f"  â³ {len(missing)} CUSIPs still need lookup")
    
    df.to_csv(input_file, index=False)
    print(f"  ðŸ’¾ Saved intermediate results to {input_file.name}")
...
def _lookup_web(tasks: List[Tuple[int, str]]) -> Dict[int, str]:
    ...
    try:
        ...
        print("  âš ï¸ Playwright not installed, skipping web lookup")
        return {}
...
def main():
    ...
    from step1_russell3000_filtering.config import DEFAULT_REFERENCE_FILE
    ...
    if not input_file.exists():
        sys.exit(f"âŒ Input file not found: {input_file}")
    if not ref_file.exists():
        sys.exit(f"âŒ Reference file not found: {ref_file}")
