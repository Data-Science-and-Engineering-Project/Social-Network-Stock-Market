"""
Stock Market Social Network - ADAMIC-ADAR Link Prediction
...
Score(f, s) = Σ_{f' ≠ f, f' holds s} |N(f) ∩ N(f')| / log(deg(f'))
Path: fund → stock' → fund' → stock  (length-3 bipartite path)
"""
...
log_file = os.path.join(LOG_DIR, f'adamicadar_{timestamp}.log')
...
def load_data():
    ...
    log_message("\n" + "=" * 80)
    log_message("STAGE 1: SYSTEM & PATH INITIALIZATION [ADAMIC-ADAR]")
    log_message("=" * 80)
...
def build_quarterly_graphs(data):
    ...
    for (year, quarter_str), group in data.groupby(['YEAR', 'QUARTER']):
        ...
        # FIX: zip is ~10x faster than itertuples for building edge lists
...
def get_sliding_window_splits(chronological_quarters, train_window=8, test_offset=1):
    """
    Generate temporally correct train/test splits with no leakage.
...
def get_shared_universe(G_train, G_test):
    ...
    Only entities that appear in BOTH last-train and test quarters are evaluated,
    ensuring fair comparison across windows.
    """
...
def generate_test_samples(G_test, G_last_train, num_negatives_ratio=1.0, seed=42):
    ...
    # Shared universe: only entities seen in both last-train and test quarters
    ...
    # Positive edges: only within shared universe
    ...
    # FIX: numpy batch random — generate all candidates at once, then filter
    ...
    neg_edges = []
    seen_neg  = set()
    oversample_factor = 5  # generate 5× candidates to handle collisions
    ...
    while len(neg_edges) < num_neg_target:
        ...
        # Safety exit if graph is too dense to find enough negatives
...
def compute_ranking_metrics(test_pairs, y_true, y_prob, k_list=(5, 10, 20, 50)):
    ...
    For each fund in the test set:
      • Rank all its candidate stocks by AA score (descending)
      • Hit@k  : 1 if ANY true-positive stock appears in the top-k, else 0
      • NDCG@k : DCG@k / IDCG@k  (binary relevance)
    Returns averages over all funds that have at least one positive.
    """
    # ── Group by fund ──────────────────────────────────────────────────────────
    fund_positives = {}   # fund → set of positive stocks
    fund_scores    = {}   # fund → {stock: score}
...
def evaluate_all_metrics(y_true, y_prob, test_pairs, model_name="Adamic-Adar"):
    """
    Compute the full metrics suite required for comparison with learned models.

    Columns produced (matches the learned-model output format):
      final_train_loss, final_val_loss, best_val_loss  — NaN (no optimization)
      test_auc, test_avg_precision
      opt_threshold                                     — maximises F1 on test
      test_precision_05, test_recall_05, test_f1_05     — fixed threshold 0.5
      test_precision_opt, test_recall_opt, test_f1_opt  — oracle-optimal threshold
      hit5, hit10, hit20, hit50
      ndcg5, ndcg10, ndcg20, ndcg50
      rank_return_spearman                              — Spearman(score, y_true)

    Note on rank_return_spearman:
      Ideal usage requires actual stock return data alongside y_true.
      Here y_true (1 = fund holds stock) serves as the return proxy; swap for
      real returns when available.
    """
    ...
    test_auc           = roc_auc_score(y_true, y_prob)          if has_both_classes else 0.5
    test_avg_precision = average_precision_score(y_true, y_prob) if has_both_classes else 0.0

    # ── 2. Optimal threshold (oracle: maximises F1 on the test set) ───────────
    if has_both_classes:
        prec_c, rec_c, thr_c = precision_recall_curve(y_true, y_prob)
        # prec_c / rec_c have n_thresholds+1 elements; thr_c has n_thresholds
        f1_c      = np.where(
            (prec_c[:-1] + rec_c[:-1]) > 0,
        ...
        # Read precision/recall/f1 directly from the curve (avoids recompute)
...
def adamic_adar_score(train_quarters, test_pairs, quarterly_graphs,
                      shared_funds=None, shared_stocks=None):
    """
    Bipartite Adamic-Adar — fully vectorized via sparse matrix multiplication.

    Formula (length-3 path: fund → stock' → fund' → stock):
        Score(f, s) = Σ_{f' ≠ f, f' holds s}  |N(f) ∩ N(f')| / log(deg(f'))

    In matrix form (A = Funds × Stocks bi-adjacency, W = diag(1/log(deg_fund))):
        S = (A · Aᵀ - diag) · W · A
        S[f, s] = Σ_{f'} common_stocks(f, f') · w(f') · A[f', s]

    Threshold: median of scores — fixed split point (50th percentile), no leakage,
    consistent across windows, appropriate for balanced 1:1 sampling.

    NOTE: All computation uses TRAINING data only. test_pairs are only used
    to look up pre-computed scores — no test information enters the model.
    """
    log_message("    [-] Computing Vectorized Bipartite Adamic-Adar scores...")

    # ── Collect all node IDs from training quarters ──────────────────────────
    all_fund_ids  = set()
    all_stock_ids = set()
    ...
    # Apply shared universe filter (no new information — just restricts evaluation scope)
    ...
    if not funds or not stocks:
        log_message("    [!] Error: Empty fund/stock universe. Returning zeros.")
        return [0.0] * len(test_pairs)

    fund_to_idx  = {f: i for i, f in enumerate(funds)}
    stock_to_idx = {s: j for j, s in enumerate(stocks)}

    # ── Build sparse bi-adjacency matrix A (Funds × Stocks) ─────────────────
    # FIX: build directly from quarterly graphs — no intermediate NetworkX G_train_agg
    ...
    if not row_indices:
        log_message("    [!] Error: No edges found in training window. Returning zeros.")
        return [0.0] * len(test_pairs)
    ...
    # Binarize: multiple train quarters may repeat same edge — presence-only
    A = (A > 0).astype(np.float32)

    # ── Adamic-Adar weights: 1 / log(fund_degree) ───────────────────────────
    fund_degrees = np.array(A.sum(axis=1)).flatten()
    with np.errstate(divide='ignore'):
        weights = np.where(fund_degrees > 1, 1.0 / np.log(fund_degrees), 0.0)
    W = diags(weights.astype(np.float32))

    # ── Sparse matrix chain: S = (A·Aᵀ - diag) · W · A ─────────────────────
    B = A.dot(A.T)      # B[f, f'] = number of common stocks (Funds × Funds)
    B.setdiag(0)        # Remove self-loops (fund is not its own co-investor)
    B.eliminate_zeros()

    C = B.dot(W)        # Scale each fund-column by its Adamic-Adar weight
    S = C.dot(A)        # Project back to stock space: Score matrix (Funds × Stocks)

    # ── Extract scores for test pairs — vectorized numpy indexing ────────────
    fund_idxs  = np.fromiter((fund_to_idx.get(f, -1)  for f, s in test_pairs), dtype=np.int32)
    stock_idxs = np.fromiter((stock_to_idx.get(s, -1) for f, s in test_pairs), dtype=np.int32)

    valid = (fund_idxs >= 0) & (stock_idxs >= 0)
    scores = np.zeros(len(test_pairs), dtype=np.float64)

    if valid.any():
        vf = fund_idxs[valid]
        vs = stock_idxs[valid]
        scores[valid] = np.asarray(S[vf, vs]).flatten()
    ...
    log_message(f"    [✓] Vectorized Adamic-Adar computed. "
...
def process_sliding_window_baselines(quarterly_graphs, train_window=8, test_offset=1):
    log_message("\n" + "=" * 80)
    log_message(f"STAGE 4: RUNNING ADAMIC-ADAR MODEL OVER SLIDING WINDOWS")
    ...
    for window_idx, (train_quarters, test_quarter) in enumerate(splits):
        ...
        result = generate_test_samples(G_test, G_last_train, num_negatives_ratio=1.0,
                                       seed=window_idx)  # deterministic per window
        if len(result) == 4:
        ...
        # Step 2: Adamic-Adar scoring (training data only)
        log_message("\n  [*] Step 2: Executing Adamic-Adar Link Prediction...")
        y_prob_aa = adamic_adar_score(
        ...
        metrics = evaluate_all_metrics(y_true, y_prob_aa, test_pairs, "Adamic-Adar")

        log_message("\n" + "." * 60)
        log_message(f"  [METRICS SUMMARY FOR WINDOW {window_idx + 1}]")
        log_message("  ADAMIC-ADAR MODEL:")
        ...
        del test_pairs, y_true, y_prob_aa, metrics
        gc.collect()
...
if __name__ == "__main__":
    log_message(">>> INITIATING ADAMIC-ADAR BASELINE EXPERIMENT <<<")
    ...
    if data is not None:
        ...
        if not baseline_results_df.empty:
            log_message("\n\n" + "=" * 80)
            log_message("FINAL AGGREGATE EVALUATION SUMMARY - ADAMIC-ADAR")
            log_message("=" * 80)
            ...
            log_message("ADAMIC-ADAR BASELINE:")
            ...
            # Save results
            output_csv = os.path.join(RESULTS_DIR, f'adamicadar_scores_{timestamp}.csv')
