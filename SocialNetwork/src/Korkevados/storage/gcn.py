#!/usr/bin/env python
"""sweep_features_v4_gcn.py - Train-on-Q, target-Q+1 GCN sweep.

GCN counterpart to sweep_features_v4.py (which uses a LightGCN-style mean
pool over all layer outputs with no nonlinearity). This version is a
"proper" GCN:
  - GCNConv stack with ReLU activations + dropout between layers
  - Final embedding = output of the LAST conv layer (no layer-mean pool)
  - Self-loops + improved normalization on by default

Everything else (Q -> Q+1 split, BPR loss, evaluation, per-stock ranking,
resumable I/O, signal handling) mirrors sweep_features_v4.py.

For each consecutive pair (Q, Q+1):
  - input graph        = Q's delta-edges with delta_w > 0 (BUYS only)
  - target positives   = Q+1's delta-edges with delta_w > 0, restricted to
                         the (Q intersect Q+1) universe
  ...
  - Per-stock ranking = mean sigma(fund . stock) across all Q-funds for each
    Q-stock (tagged predicts_year = Q+1)
  - Spearman corr(rank, Q+1 actual returns)
  - Best (highest test_auc) model state_dict is checkpointed to disk

Outputs (in --results-dir, default <script>/results/GcnV4):
  - sweep_results_v4_gcn__<edges-col>.csv         one row per (Q, Q+1) pair
  - cusip_ranks_v4_gcn__<edges-col>.parquet       per-stock per-pair ranks
  - best_model_v4_gcn__<edges-col>.pth            state_dict of best model
  - best_model_v4_gcn__<edges-col>.meta.json      metadata about that best model

Usage:
    python sweep_features_v4_gcn.py
    python sweep_features_v4_gcn.py --edges-col change_in_adjusted_weight
    python sweep_features_v4_gcn.py --quarters 2024Q1,2024Q2 --data-dir ../../data
"""
import argparse
import json
import os
...
def get_args():
    ...
    p.add_argument("--results-dir", default=None,
                   help="Output dir. Default: $FGNN_RESULTS_DIR -> <script>/results/GcnV4")
    p.add_argument("--epochs",        type=int,   default=300)
    p.add_argument("--embed-dim",     type=int,   default=128)
    p.add_argument("--num-layers",    type=int,   default=3)
    p.add_argument("--dropout",       type=float, default=0.2,
                   help="Dropout between GCN layers (applied to non-final layers)")
    ...
    p.add_argument("--model-tag",     default="WeightedGCN_v4",
...
def build_feature_graph(year, quarter, col, loaders):
    """Build the INPUT graph for quarter Q (sign-filtered to delta_w > 0)."""
    edges = loaders["load_edges"](year, quarter, col, positive_only=True)
    if edges.empty:
        raise RuntimeError(f"no positive delta-edges for {year} Q{quarter}")
...
def build_target_pairs(next_year, next_quarter, edges_col, G, loaders):
    """Load Q+1's positive delta-edges, restrict to (Q intersect Q+1) shared
    universe, and map them to Q's local node indices.
...
class WeightedGCN(nn.Module):
    """Proper GCN: GCNConv stack with ReLU + dropout between layers; the
    final embedding is the LAST layer's output (no LightGCN-style layer
    averaging). Edge weights are used by GCNConv's normalization."""

    def __init__(self, in_feats, embedding_dim, num_layers, dropout=0.2):
        super().__init__()
...
def compute_stock_ranking(model, X, edge_index, edge_weight,
    ...
    """For each Q-stock, mean sigma(fund . stock) across all Q-funds.
...
def run_quarter_pair(year, quarter, args, loaders, device):
    ...
    model = WeightedGCN(G["X"].shape[1], args.embed_dim, args.num_layers,
...
def resolve_results_dir(arg_value):
    if arg_value:
        ...
        d = (Path(__file__).parent / "results" / "GcnV4").resolve()
...
def main():
    ...
    print(f"=== sweep_features_v4_gcn.py ===", flush=True)
    ...
    print(f"embed_dim:      {args.embed_dim}  layers: {args.num_layers}  dropout: {args.dropout}", flush=True)
    ...
    results_csv  = results_dir / f"sweep_results_v4_gcn__{args.edges_col}.csv"
    ranks_parquet = results_dir / f"cusip_ranks_v4_gcn__{args.edges_col}.parquet"
    best_pth     = results_dir / f"best_model_v4_gcn__{args.edges_col}.pth"
    best_meta    = results_dir / f"best_model_v4_gcn__{args.edges_col}.meta.json"
