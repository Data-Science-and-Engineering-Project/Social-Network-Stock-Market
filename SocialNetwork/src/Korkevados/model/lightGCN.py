...
from cik_profile import build_cik_profile_upto, tag_archetypes, filter_ciks
...
def get_args():
    ...
    # CIK behavioural filters — per-quarter expanding-window profile.
    # All None by default => use every CIK in the quarter (original behaviour).
    p.add_argument("--archetype", default=None,
                   choices=["buy_and_hold", "high_churn",
                            "concentrated", "diversified"],
                   help="Restrict training CIKs to one behavioural archetype "
                        "(quantile-based, recomputed per quarter)")
...
def make_loaders(data_dir):
    ...
    return {
        ...
        # Raw panels exposed for cik_profile.build_cik_profile_upto.
        "raw_norm_holdings": NORM,
        "raw_cik_aum":       AUM,
    }
...
def _has_active_cik_filter(args):
    if args is None:
        return False
    return any(getattr(args, name, None) is not None for name in (
        "archetype", "min_turnover", "max_turnover", "max_open_rate",
        "min_avg_duration", "max_avg_duration",
        "min_n_holdings", "max_n_holdings",
    ))


def allowed_ciks_for_quarter(year, quarter, args, loaders):
    """Compute the expanding-window CIK profile up through (year, quarter)
    and apply the user's filter args. Returns None when no filter flag is
    set (preserves original 'use all CIKs' behaviour)."""
    if not _has_active_cik_filter(args):
        return None
    profile = build_cik_profile_upto(year, quarter,
                                     loaders["raw_norm_holdings"],
                                     loaders["raw_cik_aum"])
    ...
    return filter_ciks(
        ...
        max_open_rate=args.max_open_rate,
        min_avg_duration=args.min_avg_duration,
        max_avg_duration=args.max_avg_duration,
        min_n_holdings=args.min_n_holdings,
        max_n_holdings=args.max_n_holdings,
    )


def build_feature_graph(year, quarter, col, loaders, args=None):
    """Build the INPUT graph for quarter Q (sign-filtered to Δw > 0).

    If `args` carries any active CIK-filter flag, edges whose CIK falls
    outside the allowed set are dropped before node-index assignment."""
    edges = loaders["load_edges"](year, quarter, col, positive_only=True)
    if edges.empty:
        raise RuntimeError(f"no positive Δ-edges for {year} Q{quarter}")

    allowed = allowed_ciks_for_quarter(year, quarter, args, loaders)
    if allowed is not None:
        before_ciks = edges["cik"].nunique()
        before_edges = len(edges)
        edges = edges[edges["cik"].isin(allowed)].reset_index(drop=True)
        after_ciks = edges["cik"].nunique()
        print(f"  [filter] {year} Q{quarter}: CIKs {before_ciks} -> "
              f"{after_ciks}  edges {before_edges:,} -> {len(edges):,}",
              flush=True)
        if edges.empty:
            raise RuntimeError(
                f"all edges filtered out for {year} Q{quarter} "
                f"— relax CIK filter thresholds")
...
def run_quarter_pair(year, quarter, args, loaders, device):
    ...
    G = build_feature_graph(year, quarter, args.edges_col, loaders, args)
    ...
    metrics = {
        ...
        **rrc,
        # CIK filter settings recorded so different runs are distinguishable
        # in the sweep CSV.
        "filter_archetype":         args.archetype if args.archetype else "",
        "filter_min_turnover":      args.min_turnover,
        "filter_max_turnover":      args.max_turnover,
        "filter_max_open_rate":     args.max_open_rate,
        "filter_min_avg_duration":  args.min_avg_duration,
        "filter_max_avg_duration":  args.max_avg_duration,
        "filter_min_n_holdings":    args.min_n_holdings,
        "filter_max_n_holdings":    args.max_n_holdings,
...
def save_best_model(state_dict, metrics, args, pth_path, meta_path):
    ...
    meta = {
        ...
        "rank_return_spearman": float(metrics.get("rank_return_spearman",
                                                  float("nan"))),
        "filter_archetype":         args.archetype,
        "filter_min_turnover":      args.min_turnover,
        "filter_max_turnover":      args.max_turnover,
        "filter_max_open_rate":     args.max_open_rate,
        "filter_min_avg_duration":  args.min_avg_duration,
