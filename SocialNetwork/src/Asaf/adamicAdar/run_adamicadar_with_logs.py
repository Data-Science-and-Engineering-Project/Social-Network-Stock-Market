...
def process_sliding_window_baselines(quarterly_graphs, train_window=8, test_offset=1):
    ...
    for window_idx, (train_quarters, test_quarter) in enumerate(splits):
        ...
        y_prob_aa, y_pred_aa = adamic_adar_score(G_test, train_quarters, test_pairs, quarterly_graphs)
        res_aa = evaluate_predictions(y_true, y_prob_aa, y_pred_aa, "Adamic-Adar")
        
        log_message("\n" + "." * 60)
        log_message(f"  [METRICS SUMMARY FOR WINDOW {window_idx + 1}]")
        log_message("  ADAMIC-ADAR MODEL:")
        log_message(f"  -> AUC-ROC:  {res_aa['AUC']:.4f}")
        log_message(f"  -> Precision:{res_aa['Precision']:.4f}")
        log_message(f"  -> Recall:   {res_aa['Recall']:.4f}")
        log_message(f"  -> F1-Score: {res_aa['F1']:.4f}")
        log_message("." * 60)
        ...
        all_results.append({
            ...
            'aa_auc': res_aa['AUC'], 
            'aa_precision': res_aa['Precision'], 
            'aa_recall': res_aa['Recall'], 
            'aa_f1': res_aa['F1']
        })
        
        log_message(f"\n  [*] Step 3: Window {window_idx + 1} memory cleanup...")
        del test_pairs, y_true, y_prob_aa, y_pred_aa
        gc.collect()
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        if not baseline_results_df.empty:
            ...
            display_cols = ['window', 'test_quarter', 'aa_auc', 'aa_f1']
            ...
            log_message(f"  Average AUC:       {baseline_results_df['aa_auc'].mean():.4f} (±{baseline_results_df['aa_auc'].std():.4f})")
            log_message(f"  Average Precision: {baseline_results_df['aa_precision'].mean():.4f} (±{baseline_results_df['aa_precision'].std():.4f})")
            log_message(f"  Average Recall:    {baseline_results_df['aa_recall'].mean():.4f} (±{baseline_results_df['aa_recall'].std():.4f})")
            log_message(f"  Average F1-Score:  {baseline_results_df['aa_f1'].mean():.4f} (±{baseline_results_df['aa_f1'].std():.4f})")
            log_message("*" * 45)
