"""
Stock Market Social Network - KATZ INDEX Link Prediction
Lightweight script for weak servers with EXTENSIVE LOGGING.
Katz Index: Sum of all paths between two nodes weighted by path length.
Score(A,B) = Σ β^l * (number of paths of length l)
"""
...
log_file = os.path.join(LOG_DIR, f'katzindex_{timestamp}.log')
...
def load_data():
    ...
    log_message("\n" + "=" * 80)
    log_message("STAGE 1: SYSTEM & PATH INITIALIZATION [KATZ INDEX]")
    log_message("=" * 80)
...
# ============================================================================
# 4. KATZ INDEX LINK PREDICTION
# ============================================================================

def katz_index_score(G, train_quarters, test_pairs, quarterly_graphs, beta=0.001, max_length=3):
    """
    Katz Index: Sum of weighted paths between nodes.
    Score(A,B) = Σ β^l * (number of paths of length l)
    Using simplified computation: count paths up to max_length
    """
    log_message(f"    [-] Computing Katz Index scores (β={beta}, max_length={max_length})...")
    ...
    for f, s in test_pairs:
        ...
        try:
            # Try to find paths up to max_length
            if f in G_train_agg and s in G_train_agg:
                for length in range(1, max_length + 1):
                    try:
                        paths = list(nx.all_simple_paths(G_train_agg, f, s, cutoff=length))
                        # Count paths of this length
                        count = sum(1 for p in paths if len(p) - 1 == length)
                        katz_score += (beta ** length) * count
                    except nx.NetworkXNoPath:
                        pass
                    except nx.NodeNotFound:
                        break
        except Exception as e:
            log_message(f"    [!] Warning: Katz computation failed for ({f}, {s}): {str(e)}")
            katz_score = 0
        
        scores.append(katz_score)
        max_score = max(max_score, katz_score)
    ...
    log_message(f"    [✓] Katz Index computed. Max: {max_score:.6f}, Threshold: {threshold:.6f}")
...
def process_sliding_window_baselines(quarterly_graphs, train_window=8, test_offset=1):
    log_message("\n" + "=" * 80)
    log_message(f"STAGE 4: RUNNING KATZ INDEX MODEL OVER SLIDING WINDOWS")
    ...
    for window_idx, (train_quarters, test_quarter) in enumerate(splits):
        ...
        # Katz Index
        log_message("\n  [*] Step 2: Executing Katz Index Link Prediction...")
        y_prob_katz, y_pred_katz = katz_index_score(G_test, train_quarters, test_pairs, quarterly_graphs, beta=0.001, max_length=3)
        res_katz = evaluate_predictions(y_true, y_prob_katz, y_pred_katz, "Katz Index")
        
        log_message("\n" + "." * 60)
        log_message(f"  [METRICS SUMMARY FOR WINDOW {window_idx + 1}]")
        log_message("  KATZ INDEX MODEL:")
        log_message(f"  -> AUC-ROC:  {res_katz['AUC']:.4f}")
        log_message(f"  -> Precision:{res_katz['Precision']:.4f}")
        log_message(f"  -> Recall:   {res_katz['Recall']:.4f}")
        log_message(f"  -> F1-Score: {res_katz['F1']:.4f}")
        log_message("." * 60)
        
        all_results.append({
            'window': window_idx + 1,
            'test_quarter': f"{test_year}Q{test_quarter_num}",
            'katz_auc': res_katz['AUC'], 
            'katz_precision': res_katz['Precision'], 
            'katz_recall': res_katz['Recall'], 
            'katz_f1': res_katz['F1']
        })
        
        log_message(f"\n  [*] Step 3: Window {window_idx + 1} memory cleanup...")
        del test_pairs, y_true, y_prob_katz, y_pred_katz
        gc.collect()
...
if __name__ == "__main__":
    log_message(">>> INITIATING KATZ INDEX BASELINE EXPERIMENT <<<")
    ...
    if data is not None:
        ...
        if not baseline_results_df.empty:
            log_message("\n\n" + "=" * 80)
            log_message("FINAL AGGREGATE EVALUATION SUMMARY - KATZ INDEX")
            log_message("=" * 80)
            
            display_cols = ['window', 'test_quarter', 'katz_auc', 'katz_f1']
            ...
            log_message("KATZ INDEX BASELINE:")
            log_message(f"  Average AUC:       {baseline_results_df['katz_auc'].mean():.4f} (±{baseline_results_df['katz_auc'].std():.4f})")
            log_message(f"  Average Precision: {baseline_results_df['katz_precision'].mean():.4f} (±{baseline_results_df['katz_precision'].std():.4f})")
            log_message(f"  Average Recall:    {baseline_results_df['katz_recall'].mean():.4f} (±{baseline_results_df['katz_recall'].std():.4f})")
            log_message(f"  Average F1-Score:  {baseline_results_df['katz_f1'].mean():.4f} (±{baseline_results_df['katz_f1'].std():.4f})")
            log_message("*" * 45)
            
            # Save results
            output_csv = os.path.join(RESULTS_DIR, f'katzindex_scores_{timestamp}.csv')
