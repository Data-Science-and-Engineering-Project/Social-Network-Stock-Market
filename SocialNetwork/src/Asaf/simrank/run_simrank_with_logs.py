"""
Stock Market Social Network - SIMRANK Link Prediction
Lightweight script for weak servers with EXTENSIVE LOGGING.
Simrank: Two nodes are similar if their neighbors are similar (recursive).
Approximation using iterative computation.
"""
...
log_file = os.path.join(LOG_DIR, f'simrank_{timestamp}.log')
...
def load_data():
    ...
    log_message("\n" + "=" * 80)
    log_message("STAGE 1: SYSTEM & PATH INITIALIZATION [SIMRANK]")
    log_message("=" * 80)
...
# ============================================================================
# 4. SIMRANK LINK PREDICTION (Simplified Approximation)
# ============================================================================

def simrank_score(G, train_quarters, test_pairs, quarterly_graphs, decay=0.8, max_iterations=3):
    """
    Simrank (Approximation): Recursive similarity based on neighbor similarity.
    Simplified computation: Use iterative approximation up to max_iterations.
    Score(A,B) depends on similarity of neighbors of A and B.
    """
    log_message(f"    [-] Computing Simrank scores (decay={decay}, iterations={max_iterations})...")
    ...
    all_nodes = list(G_train_agg.nodes())
    ...
    log_message(f"    [-] Building similarity matrix (Nodes: {n})...")
    
    # Iterative update
    try:
        for iteration in range(max_iterations):
            log_message(f"    [-] Simrank iteration {iteration + 1}/{max_iterations}...")
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    ...
                    if node_i in G_train_agg and node_j in G_train_agg:
                        neighbors_i = list(G_train_agg.neighbors(node_i))
                        neighbors_j = list(G_train_agg.neighbors(node_j))
                        ...
                        if neighbors_i and neighbors_j:
                            ...
                            for ni in neighbors_i:
                                for nj in neighbors_j:
                                    idx_ni = node_to_idx.get(ni)
                                    idx_nj = node_to_idx.get(nj)
                                    if idx_ni is not None and idx_nj is not None:
                                        score += sim[idx_ni, idx_nj]
                                        count += 1
                            
                            if count > 0:
                                sim_new[i, j] = decay * (score / (len(neighbors_i) * len(neighbors_j)))
                                sim_new[j, i] = sim_new[i, j]
            
            sim = sim_new
    except Exception as e:
        log_message(f"    [!] Warning: Simrank iteration failed: {str(e)}")
    
    # Extract scores for test pairs
    scores = []
    max_score = 0
    
    for f, s in test_pairs:
        if f in node_to_idx and s in node_to_idx:
            idx_f = node_to_idx[f]
            idx_s = node_to_idx[s]
            score = sim[idx_f, idx_s]
            scores.append(score)
    ...
    log_message(f"    [✓] Simrank computed. Max: {max_score:.4f}, Threshold: {threshold:.4f}")
...
def process_sliding_window_baselines(quarterly_graphs, train_window=8, test_offset=1):
    log_message("\n" + "=" * 80)
    log_message(f"STAGE 4: RUNNING SIMRANK MODEL OVER SLIDING WINDOWS")
    ...
    for window_idx, (train_quarters, test_quarter) in enumerate(splits):
        ...
        # Simrank
        log_message("\n  [*] Step 2: Executing Simrank Link Prediction...")
        y_prob_sr, y_pred_sr = simrank_score(G_test, train_quarters, test_pairs, quarterly_graphs, decay=0.8, max_iterations=3)
        res_sr = evaluate_predictions(y_true, y_prob_sr, y_pred_sr, "Simrank")
        
        log_message("\n" + "." * 60)
        log_message(f"  [METRICS SUMMARY FOR WINDOW {window_idx + 1}]")
        log_message("  SIMRANK MODEL:")
        log_message(f"  -> AUC-ROC:  {res_sr['AUC']:.4f}")
        log_message(f"  -> Precision:{res_sr['Precision']:.4f}")
        log_message(f"  -> Recall:   {res_sr['Recall']:.4f}")
        log_message(f"  -> F1-Score: {res_sr['F1']:.4f}")
        log_message("." * 60)
        ...
        all_results.append({
            ...
            'sr_auc': res_sr['AUC'], 
            'sr_precision': res_sr['Precision'], 
            'sr_recall': res_sr['Recall'], 
            'sr_f1': res_sr['F1']
        })
        
        log_message(f"\n  [*] Step 3: Window {window_idx + 1} memory cleanup...")
        del test_pairs, y_true, y_prob_sr, y_pred_sr
        gc.collect()
...
if __name__ == "__main__":
    log_message(">>> INITIATING SIMRANK BASELINE EXPERIMENT <<<")
    ...
    if data is not None:
        ...
        if not baseline_results_df.empty:
            log_message("\n\n" + "=" * 80)
            log_message("FINAL AGGREGATE EVALUATION SUMMARY - SIMRANK")
            log_message("=" * 80)
            
            display_cols = ['window', 'test_quarter', 'sr_auc', 'sr_f1']
            ...
            log_message("SIMRANK BASELINE:")
            log_message(f"  Average AUC:       {baseline_results_df['sr_auc'].mean():.4f} (±{baseline_results_df['sr_auc'].std():.4f})")
            log_message(f"  Average Precision: {baseline_results_df['sr_precision'].mean():.4f} (±{baseline_results_df['sr_precision'].std():.4f})")
            log_message(f"  Average Recall:    {baseline_results_df['sr_recall'].mean():.4f} (±{baseline_results_df['sr_recall'].std():.4f})")
            log_message(f"  Average F1-Score:  {baseline_results_df['sr_f1'].mean():.4f} (±{baseline_results_df['sr_f1'].std():.4f})")
            log_message("*" * 45)
            
            # Save results
            output_csv = os.path.join(RESULTS_DIR, f'simrank_scores_{timestamp}.csv')
