"""
Stock Market Social Network - RESOURCE ALLOCATION Link Prediction
Lightweight script for weak servers with EXTENSIVE LOGGING.
Resource Allocation: Models flow of resources through common neighbors.
Score(A,B) = Σ common_neighbors(c) / degree(c)
"""
...
log_file = os.path.join(LOG_DIR, f'resourceallocation_{timestamp}.log')
...
def load_data():
    ...
    log_message("\n" + "=" * 80)
    log_message("STAGE 1: SYSTEM & PATH INITIALIZATION [RESOURCE ALLOCATION]")
    log_message("=" * 80)
...
# ============================================================================
# 4. RESOURCE ALLOCATION LINK PREDICTION
# ============================================================================

def resource_allocation_score(G, train_quarters, test_pairs, quarterly_graphs):
    """
    Resource Allocation: Simulates resource flow through common neighbors.
    Score(A,B) = Σ 1/degree(common_neighbor)
    """
    log_message("    [-] Computing Resource Allocation scores...")
    ...
    for f, s in test_pairs:
        if f in G_train_agg and s in G_train_agg:
            ...
            max_score = max(max_score, ra_score)
        else:
            scores.append(0)
    ...
    log_message(f"    [✓] Resource Allocation computed. Max: {max_score:.4f}, Threshold: {threshold:.4f}")
...
def process_sliding_window_baselines(quarterly_graphs, train_window=8, test_offset=1):
    log_message("\n" + "=" * 80)
    log_message(f"STAGE 4: RUNNING RESOURCE ALLOCATION MODEL OVER SLIDING WINDOWS")
    ...
    for window_idx, (train_quarters, test_quarter) in enumerate(splits):
        ...
        # Resource Allocation
        log_message("\n  [*] Step 2: Executing Resource Allocation Link Prediction...")
        y_prob_ra, y_pred_ra = resource_allocation_score(G_test, train_quarters, test_pairs, quarterly_graphs)
        res_ra = evaluate_predictions(y_true, y_prob_ra, y_pred_ra, "Resource Allocation")
        
        log_message("\n" + "." * 60)
        log_message(f"  [METRICS SUMMARY FOR WINDOW {window_idx + 1}]")
        log_message("  RESOURCE ALLOCATION MODEL:")
        log_message(f"  -> AUC-ROC:  {res_ra['AUC']:.4f}")
        log_message(f"  -> Precision:{res_ra['Precision']:.4f}")
        log_message(f"  -> Recall:   {res_ra['Recall']:.4f}")
        log_message(f"  -> F1-Score: {res_ra['F1']:.4f}")
        log_message("." * 60)
        ...
        all_results.append({
            ...
            'ra_auc': res_ra['AUC'], 
            'ra_precision': res_ra['Precision'], 
            'ra_recall': res_ra['Recall'], 
            'ra_f1': res_ra['F1']
        })
        
        log_message(f"\n  [*] Step 3: Window {window_idx + 1} memory cleanup...")
        del test_pairs, y_true, y_prob_ra, y_pred_ra
        gc.collect()
...
if __name__ == "__main__":
    log_message(">>> INITIATING RESOURCE ALLOCATION BASELINE EXPERIMENT <<<")
    ...
    if data is not None:
        ...
        if not baseline_results_df.empty:
            log_message("\n\n" + "=" * 80)
            log_message("FINAL AGGREGATE EVALUATION SUMMARY - RESOURCE ALLOCATION")
            log_message("=" * 80)
            
            display_cols = ['window', 'test_quarter', 'ra_auc', 'ra_f1']
            ...
            log_message("RESOURCE ALLOCATION BASELINE:")
            log_message(f"  Average AUC:       {baseline_results_df['ra_auc'].mean():.4f} (±{baseline_results_df['ra_auc'].std():.4f})")
            log_message(f"  Average Precision: {baseline_results_df['ra_precision'].mean():.4f} (±{baseline_results_df['ra_precision'].std():.4f})")
            log_message(f"  Average Recall:    {baseline_results_df['ra_recall'].mean():.4f} (±{baseline_results_df['ra_recall'].std():.4f})")
            log_message(f"  Average F1-Score:  {baseline_results_df['ra_f1'].mean():.4f} (±{baseline_results_df['ra_f1'].std():.4f})")
            log_message("*" * 45)
            
            # Save results
            output_csv = os.path.join(RESULTS_DIR, f'resourceallocation_scores_{timestamp}.csv')
